...
async (conn, mek, m, { from, body, isOwner }) => {       
    ...
    if (config.AUTO_OFFLINE === 'true') {
        await conn.sendPresenceUpdate('unavailable', from); // Offline mode
    }

    // Aapka original kaam yahaan
    await conn.sendMessage(from, { text: "Bot is replying in offline mode." }, { quoted: mek });
